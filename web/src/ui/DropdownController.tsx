...
import { SolidExpandLess } from "../icons";
...
export const DropdownController: React.FC<{
  ...
  return (
    <React.Fragment>
      <button
        className="focus:outline-no-chrome drop-button"
        ref={referenceRef}
        onClick={() => setVisibility(!visible)}
      >
        {visible ? <SolidExpandLess /> : children}
      </button>
      {createPortal(
        <div
          className="z-20 absolute top-9 left md:right-0"
          ref={popperRef}
          {...attributes.popper}
        >
          <div
